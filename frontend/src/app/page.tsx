...
import { PageResponse, Topico, Curso } from "@/types/api";
...
export default async function Home({ searchParams }: { searchParams: Promise<{ [key: string]: string | string[] | undefined }> }) {
  ...
  if (token) {
    try {
      const resTopicos = await fetchWithAuth('/topicos');
      if (resTopicos.ok) {
        const data: PageResponse<Topico> = await resTopicos.json();
        topicos = data.content;
      } else {
        error = 'La sesión expiró o no tienes acceso. Intenta iniciar sesión nuevamente.';
      }

      const resCursos = await fetchWithAuth('/cursos');
      if (resCursos.ok) {
        const dataCursos: PageResponse<Curso> = await resCursos.json();
        cursos = dataCursos.content;
      }
    } catch (e) {
  ...
  const displayTopicos = filter === 'resueltos' 
    ? topicos.filter(t => t.estado === 'RESUELTO') 
    : topicos;
    ...
    <div className="container mx-auto px-4 py-12 max-w-5xl flex-1">
      ...
      <section className="text-center py-16 px-4 bg-gradient-to-b from-primary/10 to-transparent rounded-3xl mb-12 border border-primary/10">
        ...
        <p className="text-lg md:text-xl opacity-80 max-w-2xl mx-auto mb-8">
          Pregunta, responde y aprende sobre variados temas de coding como Java, Spring Boot, React y más.
        </p>
        ...
        <div className="flex-1">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex gap-2">
              <Link href="/?filter=todos" className={`text-sm px-3 py-1.5 rounded-full font-medium transition-colors ${filter === 'todos' ? 'bg-primary/10 text-primary dark:text-indigo-300' : 'opacity-60 hover:bg-black/5 dark:hover:bg-white/5'}`}>
                Todos
              </Link>
              <Link href="/?filter=resueltos" className={`text-sm px-3 py-1.5 rounded-full font-medium transition-colors ${filter === 'resueltos' ? 'bg-primary/10 text-primary dark:text-indigo-300' : 'opacity-60 hover:bg-black/5 dark:hover:bg-white/5'}`}>
                Resueltos
              </Link>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            ) : displayTopicos.length === 0 ? (
              <div className="text-center p-8 rounded-2xl border border-black/10 dark:border-white/10">
                <p className="opacity-70">No hay tópicos para mostrar en esta categoría.</p>
              </div>
            ) : (
              displayTopicos.map((topico) => (
                <Link href={`/topicos/${topico.id}`} key={topico.id} className="block p-6 rounded-2xl border border-black/10 dark:border-white/10 hover:border-primary/50 transition-colors bg-white dark:bg-black/20 group cursor-pointer relative overflow-hidden">
                  ...
                  <div className="flex justify-between items-start mb-3">
                    <div className="flex gap-2 items-center">
                      ...
                      <span className={`text-xs font-bold px-2 py-1 rounded-md ${topico.estado === 'RESUELTO'
        ...
        <aside className="w-full md:w-64 flex flex-col gap-6 shrink-0">
          <div className="p-5 rounded-2xl border border-black/10 dark:border-white/10 bg-white dark:bg-black/20">
            ...
            <div className="flex flex-col gap-2">
              {cursos.length === 0 ? (
                <span className="text-sm opacity-50">Cargando cursos...</span>
              ) : (
                cursos.slice(0, 5).map(curso => (
                  <Link href={`/topicos?curso=${encodeURIComponent(curso.nombre)}`} key={curso.id} className="flex justify-between items-center text-sm py-1 opacity-80 hover:text-primary hover:opacity-100 transition-colors text-left">
